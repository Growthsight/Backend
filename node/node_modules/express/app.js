...
//Http Server that serves html pages
var server = app.listen(8081, function () {

  var host = server.address().address
  var port = server.address().port
  console.log("GrowthSight Server listening at http://%s:%s", host, port)
})
...
//DATABASE SETUP
var fs = require("fs");
var userDataDb = "../../../db/userDB";
var stateDataDb = "../../../db/stateData.db";
var exists = fs.existsSync(userDataDb);
var exists2 = fs.existsSync(stateDataDb);
var insertUser = "insert into user_data values(?, ?, ?);"
...
app.post('/analyze', function (req, res) {
   ...
   console.log("Got a POST request for job creation");
   var lattitude = req.body.lattitude
   var longitude = req.body.longitude
   var usState = req.body.usState
   var email = req.body.email
	 var randID= makeid();

   console.log("Email:"+email+" long:"+longitude+" lat:"+ lattitude+ " UsState:"+usState);
   if (checkData(lattitude) && checkData(longitude) && checkData(usState) && checkData(email))
   {
  
	   
      response = { message:'Success Beginnging Analysis' };
      res.status(200).send(JSON.stringify(response));
      console.log("Success");

      execCommand = 'python data_extract.py '+lattitude+' '+longitude+' '+usState;
     // execCommand = 'python ../../../dataAnalysis/data_extract/py '+lattitude+' '+longitude+' '+usState
      ...
      exec(execCommand, function(error, stdout, stderr) {
            ...
            if (error !== null && stderr !== null ) {
                ...
                //save to db
               //when  email
               create_email(email, randID);
                var db = new sqlite3.Database(userDataDb);
                var stmt = db.prepare("INSERT INTO user_data VALUES(?, ?, ?);");
                stmt.run(email, randID, stdout);  
                db.close();  
       });
   ...
   else
   {
    	...
    	console.log("Invalid Input for analysis");
   }	 
...
app.post('/stateData/', function(req, res){
   ...
   console.log("Got a GET request for job creation");
   var usState = req.body.us_state
   var crop = req.body.crop

   console.log("STATE:"+usState+" CROP:"+crop)
      //QUERY DATABASE FOR ID
      var db = new sqlite3.Database(stateDataDb);
      usState = usState.toUpperCase()
      crop = crop.toUpperCase();
      try {
         response = []
        db.all("SELECT * FROM data_state WHERE state=\""+usState+"\" AND crop=\""+crop+"\"", function(err, rows) {  
                rows.forEach(function (row) {  
                    //console.log({year:row.year, description:row.description,value:row.value}); 
                    response.push({year:row.year, description:row.description, value:row.value});    
                })  
            res.status(200).send(JSON.stringify({state_data: response}));
            console.log(response);
                    ...
                    response = {message:"State Or Crop Does Not Exist"}
                    res.status(200).send(JSON.stringify(response));
                    console.log("State Or Crop Does Not Exist");
      } 
...
app.get('/view/:id', function (req, res) {
   ...
   var uniqueViewId = req.params.id;
   
  console.log("id ="+uniqueViewId);
  if(uniqueViewId === "" || uniqueViewId=== null || uniqueViewId==="null" || uniqueViewId.length < 9)
   {
   ...
   else
   {
      //QUERY DATABASE FOR ID
      var db = new sqlite3.Database(userDataDb);
       //db.each("SELECT row AS data FROM user_data WHERE genId=\""+uniqueViewId+"\"", function(err, row) {
       //   console.log(row.email + ": " + row.data);
       //});
try {
  db.all("SELECT data FROM user_data WHERE genId=\""+uniqueViewId+"\"", function(err, rows) {  
          rows.forEach(function (row) {  
              console.log(row.data);  
              response = (row.data).replace(/(\r\n|\n|\r)/gm,"");
              res.status(200).send(JSON.stringify(response));
              console.log("worked");
          })  
        });   
...
var send_email = function(email,html)
{

      execCommand = 'echo "'+html+'" | ./mailgun -F from=\'Growthsight <Growthsight@sparakis.com>\' -F to='+email+' -F subject=\'Your GrowthSight Report Is Ready...\'';
      var exec = require('child_process').exec;
    exec(execCommand, function(error, stdout, stderr) {
    console.log('stdout: ' + stdout);
    console.log('stderr: ' + stderr);
...
var send_error_email = function(email,html)
{

      execCommand = 'echo "'+html+'" | ./mailgun -F from=\'Growthsight <Growthsight@sparakis.com>\' -F to='+email+' -F subject=\'Your GrowthSight Report Failed To Generate...\'';
      var exec = require('child_process').exec;
    exec(execCommand, function(error, stdout, stderr) {
    console.log('stdout: ' + stdout);
    console.log('stderr: ' + stderr);
...
var create_error_email = function(email){
    
 
    html ="There was an error proccesing your GrowthSight Report... Please Resubmit."
    
    send_error_email(email, html);
